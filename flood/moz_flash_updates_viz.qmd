---
title: "Moz Flash Response Floods"
format: html
editor: visual
---
...
rivers<- st_read(rivers_gpkg,"hotosm_moz_rivers_lines_geojson__hotosm_moz_waterways_lines_geojsongeojson")
...
r_fs_sfed_crop <- crop(lr_bounded$sfed, r_wp$Mozambique)
ext(r_fs_sfed_crop) <- ext( r_wp$Mozambique)

cat("resampling floodscan to worldpop\n")
# r_fs_resampled <- terra::resample(x = r_fs_sfed_crop,y=r_wp$Mozambique)
r_fs_resampled_nn <- terra::resample(
  x = r_fs_sfed_crop,r_wp$Mozambique,
  method ="near"
  )
r_fs_resampled_bilinear <- terra::resample(x = r_fs_resampled,
                                           y=r_wp$Mozambique, 
                                           method ="bilinear")

plot(r_fs_sfed_crop[[1]])
plot(r_fs_resampled_nn[[1]])
plot(r_fs_resampled_bilinear[[1]])
...
lr_exposure <- lr_bounded %>% 
  map(
    \(rtmp){
      ...
      cat("resampling floodscan to worldpop\n")
      r_fs_resampled <- terra::resample(x = r_fs_max_crop,r_wp$Mozambique,method="near")
