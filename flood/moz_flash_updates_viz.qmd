...
lr_exposure_daily <- lr_bounded %>% 
  map(
    \(rtmp){
      cat("cropping\n")
      r_fs_tmp <- crop(rtmp, r_wp$Mozambique)
      ext(r_fs_tmp) <- ext( r_wp$Mozambique)
...
l_ps <-adm1_w_exposure %>% 
  map(
    \(adm1_tmp){
      ...
      length(unique(df_tmp$adm2_en))
      num_legend_cols <-  ifelse(length(unique(df_tmp$adm2_en))>6,2,1)
      p <- df_tmp %>% 
        ggplot(
        ...
        scale_color_manual(values = tmaptools::get_brewer_pal("Accent", n = 10,plot=F))+
        scale_fill_manual(values = tmaptools::get_brewer_pal("Accent", n = 10,plot=F))+
