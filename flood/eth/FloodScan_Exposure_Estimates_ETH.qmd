...
tar_load(lgt_eth_ssn_adm1_bin_method,store = here("flood_target_store"))
tar_load(lgt_eth_ssn_adm2_bin_method,store = here("flood_target_store"))
tar_load(lgdf_adm,store = here("flood_target_store"))
tar_load(df_eth_ssn_adm1_mean_stat,store = here("flood_target_store"))
tar_load(df_eth_ssn_adm2_mean_stat,store = here("flood_target_store"))
tar_load(df_eth_ssn_adm2_stats_bin_method,store = here("flood_target_store"))
```
...
```{r ,eval=T}
lgt_eth_ssn_adm1_bin_method$Ethiopia
```
...
gdf_adm1_stat<- lgdf_adm$adm1 %>% 
  ...
  left_join(df_eth_ssn_adm1_mean_stat)
...
```{r ,eval=T}
lgt_eth_ssn_adm2_bin_method$Ethiopia
```
...
gdf_adm2_stat<- lgdf_adm$adm2 %>% 
  filter(adm0_pcode == "ET") %>%
  left_join(df_eth_ssn_adm2_mean_stat)
...
Calculating range estimates using the confidence interval at admin 2.
...
adm0_map <- df_eth_ssn_adm2_stats_bin_method$adm0_en %>% unique()
...
p_adm2_distribution_estimates$Ethiopia
