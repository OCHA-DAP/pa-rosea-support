...
subtitle: "Analysis of Sofala & Inhambane Provinces"
format:
  html:
    toc: true
    toc-depth: 3
...
# seems like open issues w/ quarto and logical boolean/params. Just going to run this
# switch so I don' thave to change code
update_data <- switch(
  params$update_data,
  "true" = TRUE,
  "false"= FALSE
...
l_ps <-set_names(adm1_w_exposure,adm1_w_exposure) %>% 
  map(
    \(adm1_tmp){
      
      all_dates <- unique(df_adm2_exposed_daily_processed_sfed_mfed$date)
      
      df_tmp_filt <- df_adm2_exposed_daily_processed_sfed_mfed %>% 
        select(-any_of(c("stat"))) %>% 
        filter(
          adm1_en == adm1_tmp
        ) %>% 
        group_by(across(matches("^adm\\d"))) %>%
        complete(
          
            data_source = c("SFED", "MFED"),
            date= all_dates,
          
          
          fill = list(pop_exposed = 0)
        )
      ...
      df_tmp <- df_tmp_filt%>%
       ...
       df_tmp_filt_wide <- df_tmp_filt%>%
        pivot_wider(names_from = data_source, values_from=pop_exposed)
      
      df_tmp <- df_tmp %>% 
        left_join(df_tmp_filt_wide) %>% 
        mutate(
          across(c("SFED","MFED"),~replace_na(.x,0))
        )
      ...
      p <- df_tmp %>% 
        ...
        geom_point_interactive(alpha=0.8,aes(tooltip= glue("District:{ adm2_en}
                                                          ...
                                                          pop exposed (MFED): {format(round(MFED,0),nsmall=0,big.mark=',')}
                                                          pop exposed (SFED): {format(round(SFED,0),nsmall=0,big.mark=',')}"
                                                           )))+
        # geom_line(alpha=0.7)+
...
adm1_simp<- st_simplify(adm1_aoi,20000)
adm2_simp <- st_simplify(lgdf_moz$adm2 ,dTolerance = 1000)
...
ggplot()+
        geom_sf(data= adm0_simp,fill="white", linewidth=0.8)+
        geom_sf(data= adm1_simp,fill=NA,linewidth=0.5,color="grey")+
        # geom_sf(data= adm2_simp,fill=NA,linewidth=0.2,color="lightgrey")+
        # geom_spatraster(data = lr_bounded$sfed)+
  facet_wrap(~adm1_en,scales="free")+
   geom_spatraster(data = lr_bounded$sfed[[2]])+
        scale_fill_steps2(
...
adm1_iter <- unique(adm1_simp$adm1_en) 

lp_gifs <- set_names(adm1_iter,adm1_iter)%>% 
  map(
    \(adm1_tmp){
      
      df_map_tmp <- adm1_simp %>% 
        filter(
          adm1_en == adm1_tmp
        )
      adm2_tmp <- adm2_simp %>% 
      ...
      df_map_tmp_bbox <-  st_bbox(df_map_tmp)
      
      ggplot()+
        geom_sf(data= adm0_simp,fill="white", linewidth=0.8)+
        geom_sf(data= df_map_tmp,fill=NA,linewidth=0.5,color="grey")+
        geom_sf(data= adm2_tmp,fill=NA,linewidth=0.2,color="lightgrey")+
        geom_spatraster(data = lr_bounded$sfed[[1:4]])+
        scale_fill_steps2(
          breaks = seq(0,1,by=0.2),
          low = low_fill,
          mid = med_fill,
        ...
        geom_sf(
          ...
          alpha=0.8, 
          color = "white"  )+
        coord_sf(xlim=df_map_tmp_bbox[c(1,3)],ylim=df_map_tmp_bbox[c(2,4)])+
        labs(title = paste0("Mozambique - ",adm1_tmp," Province: Daily Flood Fraction (SFED)"),
...
# lp_gifs[1]
# gif_1 <- animate(lp_gifs[[1]],width=500, height = 500)
# gif_2 <- animate(lp_gifs[[2]],width=500, height = 500)

gif_1 <- animate(lp_gifs$Inhambane,width=500, height = 500)
gif_2 <- animate(lp_gifs$Sofala,width=500, height = 500)

# length(lp_gifs[[1]])
# length(lp_gifs[[2]])
mgif_1 <-  image_read(gif_1)
mgif_2 <- image_read(gif_2)

new_gif <-  image_append(c(mgif_1[1], mgif_2[1]))
for(i in 2:14){
  combined_mgif <- image_append(c(mgif_1[i],mgif_2[i]))
  new_gif <- c(new_gif, combined_mgif) 
}
new_gif
# length(lp_gifs)
# patchwork::wrap_plots(lp_gifs,ncol = 2)
