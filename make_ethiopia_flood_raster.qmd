...
Currently we have produced estimates of population exposed to flooding at admins 0-2 for Ethiopia for the Gu season. The methodology can be found [here](insert%20link). These outputs are tabular and therefore well suited for tables and choropleth maps. However, to make a more interesting and potentially useful spatial maps we should also produce raster approximations.

Therefore, this document creates the raster approximations for: - Flooded fraction (historical mean + median) - Flood Exposure (flood fraction mean/median + pop density)
...
gdf_eth_adm0 <- lgdf_adm$adm0 %>% 
                       filter(adm0_en=="Ethiopia")


r_fs <- tidync(fp_fs) %>% 
    fs_filter_bounds(geometry = gdf_eth_adm0) %>% 
...
# do the same, but this time for AMJ
lr_max <- unique(fs_lookup$fs_seas) %>%
  map(
    \(fs_seas_tmp){
      rname_temp<- fs_lookup %>%
        filter(fs_seas==fs_seas_tmp) %>%
        pull(fs_name)
...
r_max <- rast(lr_max)
r_mean_ssn = mean(r_max)
r_median_ssn = median(r_max)
...
r_mean_yr <- terra::mask(x = r_mean_ssn, mask = lgdf_adm$adm0)

r_median_yr[r_median_yr<0.1] <- NA
r_median_yr <- terra::mask(x = r_median_ssn, mask = lgdf_adm$adm0)
...
ggplot()+
  ...
  geom_sf(data=(lgdf_adm$adm1 %>% filter(adm0_en=="Ethiopia")),fill=NA,linewidth=0.5,color="grey")+
  geom_sf(data=(lgdf_adm$adm0 %>% filter(adm0_en=="Ethiopia")),fill=NA, linewidth=0.8)+
